<div class="max-w-4xl mx-auto space-y-4 sm:space-y-5">
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4 sm:p-5 rounded-xl flex items-center justify-between shadow-xs">
        <div>
            <h1 class="text-base sm:text-lg font-bold text-zinc-900 dark:text-white tracking-tight">Pencatatan Penarikan Tabungan</h1>
            <p class="text-xs text-zinc-500 dark:text-zinc-400">Verifikasi identitas nasabah dan cek saldo sebelum menyerahkan uang tunai</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
        ...
        <div class="lg:col-span-2 space-y-4">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 sm:p-5 shadow-xs">
                <form wire:submit="processTarik" class="space-y-4">
                    <!-- Step 1: Select Nasabah -->
                    <div>
                        <label class="block text-xs font-semibold text-zinc-800 dark:text-zinc-200 mb-1">
                            1. Rekening Nasabah Penarik <span class="text-rose-500">*</span>
                        </label>

                        @if ($selectedNasabah)
                            <div class="p-3 rounded-lg bg-rose-50/70 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 flex items-center justify-between">
                                <div class="flex items-center gap-2.5">
                                    <div class="size-8 rounded-lg bg-rose-100 dark:bg-rose-900/60 text-rose-700 dark:text-rose-300 flex items-center justify-center font-bold text-xs">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center gap-2 text-[11px]">
                                            <span class="font-mono text-zinc-600 dark:text-zinc-400">{{ $selectedNasabah->nomor_nasabah }}</span>
                                            <span class="text-zinc-300 dark:text-zinc-600">&bull;</span>
                                            <span class="font-mono font-semibold text-emerald-600 dark:text-emerald-400">Saldo: {{ $selectedNasabah->formatted_saldo }}</span>
                                        </div>
                                    </div>
                                </div>
                                <button 
                        ...
                        @else
                            <div class="relative">
                                <input 
                                    ...
                                    class="w-full pl-8 pr-3 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700/80 rounded-lg text-xs text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-rose-500"
                                    autofocus
                                />
                                    ...
                                    @foreach ($searchResults as $result)
                                        ...
                                        >
                                            ...
                                            <div class="text-right">
                                                <span class="text-xs font-bold font-mono text-zinc-800 dark:text-zinc-200">{{ $result->formatted_saldo }}</span>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="block text-xs font-semibold text-zinc-800 dark:text-zinc-200">
                                2. Nominal Uang Ditarik (Rp) <span class="text-rose-500">*</span>
                            </label>
                        ...
                        <div class="relative">
                            ...
                            <input 
                                ...
                                step="1000"
                                placeholder="0"
                                class="w-full pl-9 pr-3 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700/80 rounded-lg text-base text-zinc-900 dark:text-white font-mono font-bold placeholder-zinc-400 focus:outline-none focus:ring-1 focus:ring-rose-500"
                            />
                        </div>
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            placeholder="Contoh: Tarik tunai simpanan"
                            class="w-full px-3 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700/80 rounded-lg text-xs text-zinc-900 dark:text-white placeholder-zinc-400 focus:outline-none focus:ring-1 focus:ring-rose-500"
                        />
                    </div>
                    ...
                    <button 
                        type="submit" 
                        class="w-full py-2.5 px-4 bg-rose-600 hover:bg-rose-500 text-white font-semibold text-xs rounded-lg shadow-xs transition-colors flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
        ...
        <div class="lg:col-span-1 space-y-3">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 shadow-xs space-y-3">
                ...
                <div class="space-y-2 text-xs">
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-rose-600 dark:text-rose-400 font-medium">- Jumlah Ditarik:</span>
                    ...
                    <div class="pt-2 border-t border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
                        <span class="font-semibold text-zinc-900 dark:text-white">Sisa Saldo Baru:</span>
                        <span class="font-mono text-sm font-bold {{ $sisa < 0 ? 'text-rose-600' : 'text-zinc-900 dark:text-white' }}">
                            @if ($selectedNasabah)
                    ...
                    @if ($sisa < 0)
                        <div class="p-2 rounded-lg bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 text-[11px]">
                            Nominal melebihi total saldo tabungan nasabah.
                        </div>
                    @endif
                </div>
                ...
                <div class="p-2.5 rounded-lg bg-zinc-50 dark:bg-zinc-950 border border-zinc-200/80 dark:border-zinc-800 text-[11px] text-zinc-500 space-y-1">
                    ...
                    <p>&bull; Periksa kecocokan data nasabah sebelum menyerahkan uang kas.</p>
                    <p>&bull; Serahkan struk resmi beserta uang tunai kepada nasabah.</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl w-full max-w-sm overflow-hidden shadow-xl text-zinc-900 dark:text-zinc-100">
                ...
                <div class="p-5 border-b border-zinc-200 dark:border-zinc-800 text-center">
                    <div class="size-9 rounded-lg bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 flex items-center justify-center mx-auto mb-2 border border-rose-200 dark:border-rose-800">
                        <x-heroicon-s-check class="size-5" />
                    </div>
                    <h3 class="text-sm font-bold text-zinc-900 dark:text-white">Penarikan Tunai Berhasil</h3>
                ...
                <div class="p-4 space-y-2.5 text-xs">
                    ...
                    <div class="flex justify-between py-1 border-b border-zinc-100 dark:border-zinc-800 text-zinc-500">
                        ...
                        <span class="font-mono font-bold text-rose-600 dark:text-rose-400 text-sm">{{ $lastTransaction->formatted_nominal }}</span>
                    </div>
                    <div class="flex justify-between py-1 border-b border-zinc-100 dark:border-zinc-800 text-zinc-500">
