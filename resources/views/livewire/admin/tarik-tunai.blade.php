    ...
    <div class="bg-zinc-900 border border-zinc-800 p-5 rounded-2xl flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-semibold text-amber-400 uppercase tracking-wider">Form Tarik Tunai</span>
            </div>
            <h1 class="text-xl font-bold text-white tracking-tight mt-1">Pencatatan Penarikan Tabungan</h1>
            <p class="text-xs text-zinc-400">Pilih rekening nasabah dan verifikasi ketersediaan saldo sebelum menyerahkan uang tunai</p>
        </div>
                ...
                <form wire:submit="processTarik" class="space-y-5">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-zinc-200 mb-1.5">
                            1. Rekening Nasabah yang Menarik Dana <span class="text-amber-400">*</span>
                        </label>

                        @if ($selectedNasabah)
                            <div class="p-3.5 rounded-xl bg-amber-950/30 border border-amber-500/40 flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div class="size-10 rounded-xl bg-amber-600/30 text-amber-400 flex items-center justify-center font-bold text-sm">
                                        {{ strtoupper(substr($selectedNasabah->nama, 0, 1)) }}
                                    </div>
                                    <div>
                                        <h4 class="text-sm font-bold text-white">{{ $selectedNasabah->nama }}</h4>
                                        <div class="flex items-center gap-2 text-xs">
                                            <span class="font-mono text-amber-400 font-semibold">{{ $selectedNasabah->nomor_nasabah }}</span>
                                            <span class="text-zinc-500">•</span>
                                            <span class="text-zinc-300 font-bold">Saldo: {{ $selectedNasabah->formatted_saldo }}</span>
                                        </div>
                                    </div>
                                </div>
                                <button 
                        ...
                        @else
                            <div class="relative">
                                <input 
                                    ...
                                    class="w-full pl-10 pr-4 py-2.5 bg-zinc-950 border border-zinc-700 rounded-xl text-xs text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-amber-500"
                                    autofocus
                                />
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="block text-xs font-bold text-zinc-200">
                                2. Nominal Uang Ditarik (Rp) <span class="text-amber-400">*</span>
                            </label>
                            @if ($selectedNasabah && (float)$selectedNasabah->saldo > 0)
                                <button 
                                    type="button" 
                                    wire:click="setAllBalance"
                                    class="text-[11px] font-semibold text-amber-400 hover:text-amber-300 underline cursor-pointer"
                                >
                                    Tarik Semua Saldo
                                </button>
                            @endif
                        ...
                        <div class="relative">
                            ...
                            <input 
                                ...
                                step="1000"
                                placeholder="0"
                                class="w-full pl-11 pr-4 py-3 bg-zinc-950 border border-zinc-700 rounded-xl text-lg text-white font-mono font-bold placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-amber-500"
                            />
                        </div>
                        ...
                        <div class="mt-2.5 flex flex-wrap items-center gap-1.5">
                            ...
                            @foreach ([50000, 100000, 200000, 500000, 1000000, 2000000] as $chip)
                                <button 
                                    ...
                                    class="px-2.5 py-1 rounded-lg bg-zinc-800 hover:bg-amber-600/30 hover:text-amber-300 text-[11px] font-mono text-zinc-300 border border-zinc-700 transition-colors"
                                >
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            class="w-full px-3.5 py-2.5 bg-zinc-950 border border-zinc-700 rounded-xl text-xs text-white placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-amber-500"
                        />
                    </div>
                    ...
                    <button 
                        type="submit" 
                        class="w-full py-3.5 px-4 bg-amber-600 hover:bg-amber-500 text-white font-bold text-sm rounded-xl shadow-lg shadow-amber-600/30 transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-[0.99]"
            ...
            <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 shadow-sm space-y-4">
                ...
                <div class="space-y-3 pt-2">
                    <div class="flex justify-between items-center text-xs">
                        <span class="text-zinc-400">Saldo Rekening Awal:</span>
                    ...
                    <div class="flex justify-between items-center text-xs">
                        <span class="text-amber-400 font-semibold">- Jumlah Ditarik:</span>
                    ...
                    <div class="pt-3 border-t border-zinc-800 flex justify-between items-center">
                        <span class="text-xs font-bold text-white">Sisa Saldo:</span>
                        <span class="font-mono text-base font-black {{ $sisa < 0 ? 'text-rose-400' : 'text-white' }}">
                            @if ($selectedNasabah)
                    ...
                    @if ($sisa < 0)
                        <div class="p-2.5 rounded-xl bg-rose-950/60 border border-rose-800 text-rose-300 text-[11px]">
                ...
                <div class="p-3 rounded-xl bg-zinc-950 border border-zinc-800 text-[11px] text-zinc-400 space-y-1">
                    <p class="font-semibold text-zinc-300">Peringatan Teller:</p>
    ...
    @if ($showSuccessModal && $lastTransaction)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
            <div class="bg-zinc-900 border border-zinc-800 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-scale-in">
                <!-- Receipt Header -->
                <div class="p-6 bg-gradient-to-b from-amber-900/50 to-zinc-900 border-b border-zinc-800 text-center relative">
                    <div class="size-12 rounded-2xl bg-amber-500/20 text-amber-400 flex items-center justify-center mx-auto mb-2.5 border border-amber-500/30">
                        <svg xmlns="http://www.w3.org/2000/svg" class="size-6" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                        </svg>
                    </div>
                    <h3 class="text-lg font-extrabold text-white">Penarikan Tunai Sukses!</h3>
                    <p class="text-xs text-zinc-400">Kode Transaksi: <strong class="text-amber-400 font-mono">{{ $lastTransaction->kode_transaksi }}</strong></p>
                </div>
                ...
                <div class="p-6 space-y-3.5 text-xs">
                    ...
                    <div class="flex justify-between py-1 border-b border-zinc-800/80">
                        <span class="text-zinc-400">Nominal Penarikan:</span>
                        <span class="font-mono font-bold text-amber-400 text-sm">{{ $lastTransaction->formatted_nominal }}</span>
                    </div>
                    <div class="flex justify-between py-1 border-b border-zinc-800/80">
                ...
                <div class="p-5 bg-zinc-950 border-t border-zinc-800 flex items-center justify-between gap-3">
                    <button 
                        ...
                        class="px-4 py-2 bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold rounded-xl flex items-center gap-2 shadow-md"
                    >
